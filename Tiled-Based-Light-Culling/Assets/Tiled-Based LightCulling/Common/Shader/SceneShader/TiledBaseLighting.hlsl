#ifndef UNIVERSAL_TILED_BASE_LIGHTING_INCLUDED
#define UNIVERSAL_TILED_BASE_LIGHTING_INCLUDED
...
#include "TiledBaseLightingInclude.hlsl"
...
Light GetAdditionalTileLight(uint i, float3 positionWS, half4 shadowMask, TileLightData data)
{
    int index = GetTileAddtionalLightIndex(data, i);
...
///////////////////////////////////////////////////////////////////////////////
half4 UniversalFragmentPBR(InputData inputData, SurfaceData surfaceData, float4 positionSS, float3 positionVS)
{
#ifdef _SPECULARHIGHLIGHTS_OFF
    bool specularHighlightsOff = true;
#else
...
#if defined(_TILED_BASE_LIGHTING)
    TileLightData tileData = GetTileLightData(positionSS, positionVS);
    for (uint lightIndex = 0u; lightIndex < tileData.lightCount; ++lightIndex)
    {
        Light light = GetAdditionalTileLight(lightIndex, inputData.positionWS, shadowMask, tileData);
        #if defined(_SCREEN_SPACE_OCCLUSION)
